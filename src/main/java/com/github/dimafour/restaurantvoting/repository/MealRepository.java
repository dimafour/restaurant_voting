...
public interface MealRepository extends BaseRepository<Meal> {
    ...
    @Query("SELECT m FROM Meal m WHERE m.restaurant.id=:restaurantId AND m.date=current_date ORDER BY m.name")
    ...
    @Modifying
    @Transactional
    @Query("DELETE FROM Meal m WHERE m.restaurant.id=:restaurantId AND m.date=current_date")
